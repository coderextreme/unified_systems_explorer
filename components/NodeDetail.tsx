...
const LoadingSpinner: React.FC = () => (
    <div className="absolute inset-0 flex flex-col items-center justify-center text-brand-text-muted bg-brand-bg">
  ...
  return (
    <div className="h-full relative">
      <div className="pb-4 border-b-2 border-brand-content mb-6">
        <h1 className="text-3xl md:text-4xl font-bold text-white">{node.name}</h1>
      ...
      <div 
        className={`prose prose-invert md:prose-lg max-w-none text-brand-text prose-headings:text-white prose-strong:text-brand-accent prose-a:text-brand-accent prose-code:text-yellow-300 prose-blockquote:border-brand-accent transition-opacity duration-300 ${isLoading ? 'opacity-0' : 'opacity-100'}`}
